...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use service role key — admin access to create users
    ...
    for (const email of emails) {
      ...
      const { data: userData, error: userError } = await supabaseAdmin.auth.admin.createUser({
        ...
        email_confirm: true, // auto confirm — no magic link needed
      ...
      results.push({ email, success: emailRes.ok, tempPassword })
    }
