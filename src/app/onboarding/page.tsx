...
export default function OnboardingPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Create probation case — org_id is optional, case must always be created
    const { data: org } = await supabase.from('organisations').select('id').eq('slug', 'logic-church').single()
    const { error: pcErr } = await supabase.from('probation_cases').insert({
      user_id: user.id,
      created_by: user.id,
      status: 'active',
    ...
    if (pcErr) console.error('Probation case error:', pcErr.message)
